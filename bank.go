...
func cmdHandler(cmd string, db *sql.DB) {
...
func dispBalance(acctId, db *sql.DB) {
    fmt.Println("--------------------------------------")
    fmt.Sprintf("Account ID: %d\n Balance: %d\n", acctId, getBalance(acctId, db,  time.Now()))
    fmt.Println("--------------------------------------")
}
...
func withdraw(acct Account, db *sql.DB, amt float64, withdrawDate time.Time) (status int) {
    ...
    if getBalance(acct, db, withdrawDate) < amt {
        status = 1
        return
    }

    // extract all valid deposits for this account
    xtns = builsXtns(acct, db)
    ...
    nullifyXtns, newXtn = idWithdrawNullXtn(xtns, withdrawAmount, withdrawDate)

    nullifyXtns(nullifyXtns, db)

    _ = deposit(newXtn.acctId, db, newXtn.amt, newXtn.xDate, newXtn.effInterestRate)
